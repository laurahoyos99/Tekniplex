...
select distinct
entity_code,entity_name,date_trunc(transaction_date,month)
--,primary_substrate
,SKU,sku_description,intercompany_transaction_tagging,customer_id,customer_name
,sum(Sales_WO_Dis_Adj) as sales_wo_dis
,sum(Sales_WO_Dis_Adj)+SUM(disc_adj) as net_sales,sum(rm_adj) as rm_adj,sum(dl_adj) as dl_adj, sum(oh_adj) as oh_adj,sum(inv_a_adj) as inv_adj
from definitive_FFNA_PR
--where date_trunc(transaction_date,month)= date('2023-03-01')
