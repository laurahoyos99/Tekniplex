...
--/*
select distinct entity_name,ppo_split,date_trunc(transaction_date,month) as month
,sku,sku_description
,customer_id,customer_name,negocio_Intercompany_Transaction_Tagging
--,primary_substrate,primary_substrate_adj
,sum(sales_adj) as sales,sum(rm_adj) as rm_adj,sum(dl_adj) as dl_adj, sum(oh_adj) as oh_adj
--,sum(Sales_w_discount) as sales_sin_pr,sum(rm_var) as rm_sin_pr,sum(dl_var) as dl_sin_pr, sum(oh_var) as oh_sin_pr
